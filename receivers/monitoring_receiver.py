"""Monitoring receiver — listens on logs and kassa.errors."""
import sys
import threading
...
def listen(timeout: float = 5.0):
    print("[Monitoring] Listening on logs + kassa.errors ...")
    received = []

    for queue in ("logs", "kassa.errors"):
        ...
        def make_callback(q):
            def callback(ch, method, props, body):
                ...
                print(f"  [Monitoring/{q}] {msg_type} — {status}")
