"""Kassa receiver — listens on kassa.incoming."""
import sys
import threading
...
def listen(timeout: float = 5.0):
    print("[Kassa] Listening on kassa.incoming ...")
    connection, channel = get_channel()
    channel.queue_declare(queue="kassa.incoming", durable=True)
    received = []
    ...
    def callback(ch, method, props, body):
        ...
        print(f"  [Kassa] {msg_type} — {status}")
        received.append({"type": msg_type, "valid": valid})
        ch.basic_ack(delivery_tag=method.delivery_tag)

    channel.basic_consume(queue="kassa.incoming", on_message_callback=callback, auto_ack=False)
